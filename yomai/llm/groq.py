...
class GroqProvider(LLMProvider):
    def __init__(self, config: LLMConfig) -> None:
        try:
            ...
            raise YomaiLLMError(
                "OpenAI SDK is required for Groq.",
                hint="Install yomai with groq support: pip install openai",
            ) from exc

        if not config.api_key:
            raise YomaiLLMError(
                "Missing required config: api_key",
                hint="Set GROQ_API_KEY or pass api_key= to LLMConfig.",
    ...
    async def stream(self, messages: list[Message], tools: list[ToolSchema], system: str) -> AsyncIterator[LLMEvent]:
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if isinstance(exc, self._openai.AuthenticationError) or not _is_transient(exc):
                    raise YomaiLLMError(str(exc), docs="https://yomai.dev/llm#groq") from exc
                last_exc = exc
        ...
        if last_exc is not None:
            raise YomaiLLMError(str(last_exc), docs="https://yomai.dev/llm#groq") from last_exc
